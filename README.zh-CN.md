...
这张网有一个边缘。路径层靠开头识别路径：斜杠、`~/`、`./` 或 `../`。直接以目录名开头的相对路径，也就是 `git status`、编译器或测试工具打印的那种写法，对路径层来说不是路径，其中的未知目录会以明文留下；`$HOME/…`、Windows 路径和 `file://` 地址里的路径也一样。词条列表里的目录在所有这些写法里都是安全的，因为词条无论名字出现在哪里都会匹配。所以这张网在绝对路径里兜住被遗忘的目录，而词条列表才是处处覆盖它的东西。
...
- 模型按它看到的东西加引号，而它看到的是一个普通单词。词条值若含有引号、反斜杠、shell 元字符（`$`、`;`、`&`、`|`、`<`、`>`、反引号）、`#`、`%`、`/` 或控制字符，被还原进为假名写下的命令行、配置行或补丁时，会改变那一行的作用。插件在启动时拒绝这样的值，并指出词条文件的行号和所发现的字符类别，从不写出值本身。只有这些字符会被拒绝：任何文字的字母，无论中文、西里尔文、土耳其文还是带变音符的拉丁文，以及数字、空格、点、连字符和括号都没有问题；CIDR 形式的网络和 `secret` 词条保留它们的斜杠，因为密钥材料和令牌是 base64，斜杠属于这个字母表；正则表达式不受检查。如果你确实需要这样的值，就把它写成一个不含该字符却能匹配它的正则表达式。
- 开启 `replace_unknown` 时，路径层只在以斜杠、`~/`、`./` 或 `../` 开头的路径里兜住未知目录。工具打印的那种没有这种开头的相对路径里，未知目录以明文留下；词条列表里的目录在任何写法里都被覆盖。见[路径与文件名](#路径与文件名)。
