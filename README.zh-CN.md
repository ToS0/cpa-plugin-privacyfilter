...
这张网有边缘。路径层靠形状识别路径。带开头标记的路径以斜杠、`~/`、`./`、`../` 或 `$HOME/` 这样的变量开头。带斜杠的裸词元，也就是 `git status`、编译器或测试工具打印路径的那种写法，在下列情况下算作路径：最后一段带文件扩展名或以斜杠结尾，第一段以点开头，或者某一段是 `-home-<user>-<project>` 形式的压平工作目录，即 Claude Code 给会话记录目录取名用的形状，这种形式没有任何斜杠也单独算作路径。所以 `kunde-x/vertrag.pdf`、`.config/kunde-x/bin`、`kunde-x/` 和 `projects/-home-user-kunde-x` 对路径层来说是路径，而 `and/or`、`km/h`、`origin/main`、`github.com/rheodev/x`、`@scope/pkg`、sed 表达式、日期和分数不是。留在网外的有：没有文件名也没有结尾斜杠的裸目录路径 `kunde-x/sub`；diff 头里的 `a/` 和 `b/` 前缀；`file://` 或 `http://` 地址里的路径；Windows 路径；内部带点的第一段 `.hidden.dir/x`，因为主机名和模块路径正是这个样子；以及直接位于根之下的压平目录 `-tmp-kunde-x`，它只在绝对形式下被兜住。这些写法里的未知目录以明文留下。词条列表里的目录在所有这些写法里都是安全的，因为词条无论名字出现在哪里都会匹配。更宽的网的代价是偶尔有形状像路径的无害词对，比如 `Input/Output.md`，它的第一个词会作为目录假名出站。

**文件名**默认不动。`README.md`、`main.go`、`config.yaml` 说的是文件是什么，它们在上百万个仓库里都一样，也是模型在目录树里定位的索引：替换它们保护不了任何东西，却让每个 `ls` 失去意义。以客户命名的文件带着客户的名字，而那个名字是词条，词条层在文件名内的词边界处找到它：`kunde-x-vertrag.pdf` 出站时是 `d-<hex>-vertrag.pdf`。没有扩展名的文件名，`Makefile`、`LICENSE`，在保留列表里；其他没有扩展名的名字按目录处理。`path.filenames: all` 把保留列表之外的每个文件名替换为 `f-<12 位十六进制><扩展名>`。
...
- 开启 `replace_unknown` 时，路径层在它按形状识别出的路径里兜住未知目录：带开头标记的路径，最后一段带扩展名或结尾斜杠、或第一段以点开头的裸路径，以及 `-home-<user>-<project>` 形式的压平工作目录。两者都没有的裸目录路径 `kunde-x/sub`、diff 头里的 `a/` 前缀、`file://` 或 `http://` 地址里的路径、Windows 路径，以及直接位于根之下的压平目录 `-tmp-kunde-x`，其中的未知目录以明文留下；词条列表里的目录在任何写法里都被覆盖。见[路径与文件名](#路径与文件名)。
