...
Entschieden am 7. September: die Lücke bleibt. Ein Text ohne führenden Schrägstrich, Punkt oder Tilde sieht in einer Werkzeugausgabe genauso aus wie eine gewöhnliche Verzeichnisstruktur eines Projekts, und der Filter kann den Unterschied nicht kennen; jede Erweiterung des Netzes ersetzt also auch harmlose Namen und verlangt eine längere Bewahrliste. Das Netz fängt das vergessene Verzeichnis in absoluten Pfaden, die Term-Liste deckt das eingetragene in jeder Schreibweise, und das README sagt beides unter den bekannten Grenzen. `TestPath_NetGapsForUnknownDirectories` bleibt als Beschreibung der Grenze stehen.

Wieder aufgemacht und zum größeren Teil kuriert am 24. September, nachdem ein Befund aus dem Betrieb die Lücke in zwei ihrer drei Formen vorgeführt hatte: der Name des Arbeitsverzeichnisses stand in bloßen relativen Pfaden und in den plattgedrückten Transkriptnamen von Claude Code im Klartext, bei denen die Schrägstriche zu Bindestrichen werden. Die Pfadebene erkennt einen Pfad seit `detect/paths.go` in dieser Fassung an seiner Form. Anker wie zuvor, dazu `$NAME/` und `${NAME}/`; ein bloßes Token mit Schrägstrichen zählt als Pfad, wenn das letzte Segment eine Endung oder einen Schrägstrich am Ende trägt, das erste mit einem Punkt beginnt oder ein Segment ein plattgedrücktes Arbeitsverzeichnis ist, ein einzelner Bindestrich, eine Wurzel aus einer festen Liste und mindestens zwei weitere Teile, was auch ohne Schrägstrich allein zählt. Ausgenommen sind ein Punkt im Inneren des ersten Segments, denn so sehen Host- und Modulpfade aus, ein einbuchstabiges erstes Segment wie in `s/x/y/`, `a/` im Diff und `I/O`, `@scope`, `//` und alles hinter `://`. Was bleibt: der bloße Verzeichnispfad ohne Datei und ohne Schrägstrich am Ende, der Diff-Kopf, `file://` und `http://`, Windows und UNC, das versteckte Verzeichnis mit Punkt im Namen und das plattgedrückte Verzeichnis direkt unter der Wurzel. Verworfen wurde die Dreisegmentregel, weil sie `net/http/httptest` und `ja/nein/vielleicht` getroffen hätte, und die Sonderbehandlung des Diff-Präfixes, weil die Einbuchstabenregel sie mit erledigt. Der einzige angenommene Fehlfang ist `Input/Output.md`, dessen erstes Wort als Verzeichnis hinausgeht. `TestPath_NetGapsForUnknownDirectories` führt jetzt achtzehn Schreibweisen vor und benennt die neun, die durchgehen; `TestPaths_BareShapes` in `detect` hält Fang und Verschonung Form für Form fest, und `TestPath_ShapesRoundTrip` bringt die neuen Formen zurück.
