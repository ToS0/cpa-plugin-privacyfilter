...
Von neunzehn Befunden sind achtzehn behoben; offen ist die Lücke im Netz der Pfadebene, und zwei Beobachtungen bleiben als Abwägung stehen, die Versalschrift mit SS und das einzelne Surrogat im Stream. Die zwei Befunde, die eine Kommandozeile verfälschen konnten, sind nach dem Vorschlag aus ihrem Kapitel kuriert: der Lader zählt Werte mit Sonderzeichen und warnt einmal; die zehn Tests des Pakets `harm` bleiben rot, weil sie die Wirkung eines solchen Wertes zeigen, und die Warnung ändert an der Wirkung nichts, nur an der Wahrscheinlichkeit. Am 7. September kamen nacheinander die sieben Kuren von je einer Zeile, die drei Lecks mit Entscheidung, die Schlüsselnamen und doppelten Schlüssel im JSON mit dem gemeinsamen Scanner, die Verfälschungen im Stream, und zuletzt die Härtung: Wortgrenzen für Regex-Terme, Adressmuster in Zahlenketten, verklebte Ersatzwerte, der Netz-Term mit kaputtem Platzhalter und die fünf Prüfungen an der Konfiguration. Die Kapitel dazu sind stehen geblieben und tragen den Vermerk, womit sie behoben wurden; das offene trägt wie zuvor seinen Test und den Vorschlag. Jedes Kapitel nennt Ursache, Fundort, Reproduktion und Vorschlag genau genug, dass eine andere Sitzung sie abarbeiten kann, ohne diese hier gelesen zu haben; die Reihenfolge steht im nächsten Kapitel.
...
Was bleibt, kann warten: die Lücken im Netz der Pfadebene, die eine Frage der Zielgruppe sind, und die Versalschrift mit SS, die eine Abwägung ist.
...
Behoben mit `65202b0`, wie vorgeschlagen: der Lader zählt die Werte, die ein Anführungszeichen, einen Rückstrich, ein Metazeichen der Shell, ein Doppelkreuz, ein Prozentzeichen, einen Schrägstrich oder ein Steuerzeichen tragen, und schreibt eine Warnung mit der Zahl, ohne die Werte. Der Schrägstrich eines Netzes in CIDR-Form zählt nicht, und ein regulärer Ausdruck wird nicht beurteilt, weil seine Metazeichen ihm gehören. Ein Startfehler ist es nicht, denn ein solcher Wert kann gewollt sein; das README nennt die Grenze. Die zehn roten Tests bleiben rot und überspringen sich weiter, denn sie zeigen die Wirkung eines solchen Wertes, und an der ändert eine Warnung nichts. Die Liste auf dem Server ist daraufhin durchgesehen und enthält keinen.
