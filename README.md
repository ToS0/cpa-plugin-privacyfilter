...
- The model quotes what it sees, and it sees a plain word. A term value that carries a quote, a backslash, a shell metacharacter (`$`, `;`, `&`, `|`, `<`, `>`, backtick), a `#`, a `%`, a `/` or a control character is restored into a command line, a configuration line or a patch that was written for the pseudonym, and changes what that line does. The plugin counts such values at start and logs one warning; a network in CIDR form is not counted for its slash. Spaces are fine, person names carry them.
